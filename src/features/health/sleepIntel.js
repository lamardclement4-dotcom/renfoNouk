...
export function sleepSeries(sleepLog, { days = 14, today } = {}) {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const e = log[date]
    const h = e ? num(e.hours) : null
    if (h != null && h > 0) out.push({ date, hours: h, quality: num(e.quality), awakenings: num(e.awakenings) || 0 })
  }
  return out
}
...
export function sleepDebt(series, need) {
  ...
  const net = Math.round((debt - surplus) * 10) / 10
  return {
...
export function sleepAnalysis(db, { days = 14, today, weeklyTrainingMins = 0 } = {}) {
  ...
  const tips = []
  if (debt && debt.net >= 5) tips.push(`Dette de ${debt.net} h accumulée sur ${debt.nights} nuits : c’est le poste de récupération à traiter en premier, avant tout complément ou protocole.`)
  else if (debt && debt.net >= 2) tips.push(`Léger déficit cumulé (${debt.net} h) : une demi-heure de plus par nuit suffirait à le résorber.`)
  if (reg && reg.level !== 'ok') tips.push(reg.text)
  if (catchUp && catchUp.flagged) tips.push(`Tu récupères ${catchUp.gap} h de plus le week-end (${catchUp.weekend} h contre ${catchUp.weekday} h en semaine) : le besoin est là toute la semaine, c’est l’occasion de dormir qui manque.`)
  if (afterTraining && afterTraining.flagged) tips.push(`Tu dors ${Math.abs(afterTraining.diff)} h de moins après une séance : vérifie l’horaire de tes entraînements tardifs et la caféine en fin de journée.`)
  if (need > BASE_NEED) tips.push(`Ton volume d’entraînement actuel élève le besoin à environ ${need} h par nuit.`)
